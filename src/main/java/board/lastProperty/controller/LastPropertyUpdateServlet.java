...
/**
 * Servlet implementation class LastPropertyUpdateServlet
 */
@WebServlet("/board/lastPropertyUpdate")
public class LastPropertyUpdateServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		request.getRequestDispatcher("/WEB-INF/views/board/lastProperty/lastPropertyUpdate.jsp")
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int result = lastPropertyService.updateBoard(board);
		// 첨부파일 삭제 처리
		if(delFiles != null) {
			for(String temp : delFiles) {
				int attachNo = Integer.parseInt(temp); // attachment pk
				BoardAttachment attach = lastPropertyService.findAttachmentByNo(attachNo);
				// a. 파일 삭제
				File delFile = new File(saveDirectory, attach.getRenameFilename());
				if(delFile.exists()) delFile.delete();

				// b. db record 삭제
				result = lastPropertyService.deleteAttachment(attachNo);
		...
		// 5. redirect
		response.sendRedirect(request.getContextPath() + "/board/lastPropertyView?no=" + no);
