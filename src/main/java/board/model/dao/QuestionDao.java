...
public class QuestionDao {
	...
	public int getTotalContents(Connection conn) {
		...
		ResultSet rset = null;
		int totalContents = 0;
		String sql = "select count(*) from su_board where board_category = 'Q'";
		try {
	...
	public List<BoardExt> findAll(Connection conn, Map<String, Object> param) {
		...
		BoardExt board = new BoardExt();
		String sql = "select * from(select row_number() over(order by board_no desc) rnum, b.board_no, b.member_id, b.board_title, b.board_date, b.board_read_count, b.board_category from su_board b) b where (rnum between ? and ?) and b.board_category = 'Q'";
	...
	public int insertBoard(Connection conn, BoardExt board) {
		int result = 0;
		PreparedStatement pstmt = null;
		String sql = "insert into su_board (board_no, member_id, board_title, board_content, board_read_count, board_category) values (seq_notice_no.nextVal, ?, ?, ?, ?, 'Q')";
