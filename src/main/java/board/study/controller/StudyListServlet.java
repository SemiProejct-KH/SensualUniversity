...
public class StudyListServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			...
			// 2. 업무로직
			// 2.a content영역
			List<BoardExt> list = studyService.findAll(param);
			System.out.println(list);
			// 2.b pagebar영역
			int totalContents = studyService.getTotalContents();
			...
			request.getRequestDispatcher("/WEB-INF/views/board/study/studyList.jsp").forward(request, response);
		} catch(Exception e) {
			e.printStackTrace();
			throw e;
		}
