 ...
 */
@WebServlet("/board/studyEnroll")
public class StudyEnrollServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		request.getRequestDispatcher("/WEB-INF/views/board/study/studyEnroll.jsp")
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			...
			// 업무로직
			int result = studyService.insertBoard(board);
			
			// 리다이렉트
			response.sendRedirect(request.getContextPath() + "/board/studyList");
		} catch(Exception e) {
			e.printStackTrace();
			throw e;
		}
